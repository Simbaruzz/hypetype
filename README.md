...
2. Сохраняем в папку, где программа будет лежать на постоянной основе и запускаем
3. Соглашаемся, что запускаем файл, скачанный из интернета и разрешаем доступ от имени Администратора
4. Кликаем правой клавишей мыши по иконке программы в трее (там где часики и дата), в открывшемся контекстном меню ставим галочку на пункте «✓Запуск при старте». «Виртуализация» уже будет активирована
5. Перезагружаемся
6. Пользуемся

 
**Удаление**
1. Идём в трей, кликаем по иконке программы правой кнопкой мыши, снимаем галочки с пунктов «Запуск при старте» и «Виртуализация»
2. Соглашаемся на получение прав Администратора
3. Перезагружаемся
...
> Никто не знает наверняка. И скорее всего будет зависеть от количества запросов. Счетчик запросов на MacOS — 15 шт · update 02.05.2025
